...
async function checkQuickBooksStatus() {
  try {
    // Check if QB tokens exist in Firestore
    const qbDoc = await getDocs(query(collection(db, 'settings'), where('__name__', '==', 'quickbooks')));
    const data = qbDoc.docs.length > 0 ? qbDoc.docs[0].data() : null;
    
    const statusDiv = document.getElementById('qb-connection-status');
    const indicator = statusDiv.querySelector('.status-indicator');
    const connectBtn = document.getElementById('qb-connect-btn');
    const syncBtn = document.getElementById('qb-sync-btn');
    const syncStatus = document.getElementById('qb-sync-status');

    if (data && data.accessToken) {
      // QuickBooks is connected!
      indicator.classList.remove('disconnected');
      indicator.classList.add('connected');
      statusDiv.querySelector('span').textContent = 'Connected to QuickBooks ✓';
      connectBtn.style.display = 'none';
      syncBtn.style.display = 'block';
      syncStatus.style.display = 'block';

      if (data.lastSync) {
        const lastSyncDate = data.lastSync.toDate ? data.lastSync.toDate() : new Date(data.lastSync);
        document.getElementById('last-sync-time').textContent = lastSyncDate.toLocaleString();
      }
      if (data.lastSyncResults) {
        document.getElementById('synced-expenses').textContent = data.lastSyncResults.expenses || 0;
        document.getElementById('synced-revenue').textContent = data.lastSyncResults.revenue || 0;
        document.getElementById('synced-accounts').textContent = data.lastSyncResults.bankAccounts || 0;
      }
    } else {
      // Not connected yet
      indicator.classList.remove('connected');
      indicator.classList.add('disconnected');
      statusDiv.querySelector('span').textContent = 'Not connected';
...
window.connectQuickBooks = function() {
  // Redirect to QuickBooks OAuth
  window.location.href = 'https://quickbooksauth-ydhz4btxpq-uc.a.run.app';
};
...
window.syncQuickBooks = async function() {
  ...
  try {
    const response = await fetch('https://quickbookssync-ydhz4btxpq-uc.a.run.app');
    ...
    if (data.success) {
      alert('✅ Sync complete!\n\n' + 
            ...
            `Bank Accounts: ${data.results.bankAccounts}`);
...
// Check if returning from QuickBooks OAuth
const urlParams = new URLSearchParams(window.location.search);
if (urlParams.get('qb_connected') === 'true') {
  alert('✅ QuickBooks connected successfully!\n\nClick "🔄 Sync Data Now" in Settings to import your financial data.');
  // Remove query parameter
  window.history.replaceState({}, document.title, window.location.pathname);
}
if (urlParams.get('qb_error')) {
  alert('❌ QuickBooks connection failed:\n\n' + decodeURIComponent(urlParams.get('qb_error')));
  window.history.replaceState({}, document.title, window.location.pathname);
}
